...
void execCreateHorizOrdinateDimension(Gui::Command* cmd) {
    ...
    if (!allVertexes.empty() && allVertexes.size() > 1) {
        ...
        for (long unsigned int n = 0; n < allVertexes.size() - 1; n++) {
            ...
            dim = _createLinDimension(objFeat, allVertexes[0].name, allVertexes[n + 1].name, "OrdinateX");
...
bool CmdTechDrawExtensionCreateHorizOrdinateDimension::isActive(void)
{
    bool havePage = DrawGuiUtil::needPage(this);
    bool haveView = DrawGuiUtil::needView(this);
    return (havePage && haveView);
...
bool CmdTechDrawExtensionCreateVertOrdinateDimension::isActive(void)
{
    bool havePage = DrawGuiUtil::needPage(this);
    bool haveView = DrawGuiUtil::needView(this);
    return (havePage && haveView);
...
Gui::Action* CmdTechDrawExtensionCreateOrdinateDimensionGroup::createAction(void)
{
    Gui::ActionGroup* pcAction = new Gui::ActionGroup(this, Gui::getMainWindow());
    pcAction->setDropDownMenu(true);
    applyCommandData(this->className(), pcAction);
...
bool CmdTechDrawExtensionCreateOrdinateDimensionGroup::isActive(void)
{
    bool havePage = DrawGuiUtil::needPage(this);
    bool haveView = DrawGuiUtil::needView(this, true);
    return (havePage && haveView);
